	...
	mCurrent(-1)
{
	...
	while (!inFile.eof())
	{
		...
		// illum
		if (GetIlluminationModel(tokens)) {
			continue;
		}
		if (mCode != EC_SUCCESSFUL) {
			break;
		...
		// Tf
		if (GetTransmissionFilter(tokens)) {
			continue;
		}
		if (mCode != EC_SUCCESSFUL) {
			break;
